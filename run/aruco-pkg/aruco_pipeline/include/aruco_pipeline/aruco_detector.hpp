...
public:
    ArucoDetector();
    explicit ArucoDetector(const std::string& yaml_path);
    ~ArucoDetector();

    ArucoDetector(const ArucoDetector& other);
    ArucoDetector& operator=(const ArucoDetector& other);
    ...
    void setParametersFromYaml(const std::string& yaml_file_path);
