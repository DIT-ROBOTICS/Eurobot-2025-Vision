...
ArucoPipeline::ArucoPipeline(const std::string &node_name) : rclcpp::Node(node_name) {
  ...
  // Pose Publisher
  blue_pose_topic_ = this->declare_parameter<std::string>("blue_pose_topic", "/aruco/blue_pose");
  yellow_pose_topic_ = this->declare_parameter<std::string>("yellow_pose_topic", "/aruco/yellow_pose");
  pub_blue_pose_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(blue_pose_topic_, 10);
  pub_yellow_pose_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(yellow_pose_topic_, 10);
...
int main(int argc, char *argv[])
{
  ...
  auto node = std::make_shared<ArucoPipeline>("aruco_pipeline_robot");
